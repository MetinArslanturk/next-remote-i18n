...
const nodeCreateClient = (config: InternalConfig, translationDeployId?: string, reInit?: boolean): CreateClientReturn => {
      ...
      config.locales.map((locale) => {
        return Promise.all(
          config.allTranslations?.map((t) => {
            return fetch(
              `${process.env.NEXT_PUBLIC_TRANSLATIONS_HOST}/${translationDeployId}/${locale}/${t}.json`
            )
              .then((res) => res.json())
              .then((d) => ({ key: t, data: d }));
          }) as any
